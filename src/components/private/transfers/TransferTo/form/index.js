...
import { sendTransaction } from "../../../../../store/actions/transactions";
...
const SendForm = ({ sendTransaction, user }) => {
  ...
  const formSchema = Yup.object().shape({
    pay_id: Yup.string()
      .required("Coloca el nombre de usuario a recibir")
      .test("pay_id", "Este nombre de pago no se encuentra registrado", async (value) => {
      ...
      .test("pay_id", "No puedes usar tu nombre de usuario como usuario a enviar", (value) =>
        user.pay_id === String(value).toLowerCase() ? false : true
      ),
    description: Yup.string().required("Agrega una descripción."),
    amount: Yup.number()
      .required("Debes colocar un monto")
      .typeError("Solo se aceptan números")
      .min(1000, "El monto mínimo aceptado es de 1000 Bs."),
            ...
            <div className="pagos-form__container">
              ...
              <div className="continue-form">
                {!submitted ? (
                  step === pages().length - 1 ? (
                    <>
                      <button type="submit" className="continue-btn button" disabled={!values.terms}>
