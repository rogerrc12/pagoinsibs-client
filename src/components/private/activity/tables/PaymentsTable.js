  ...
  return (
    <Paper style={{ overflowX: 'auto' }}>
      <Table className="user-table" aria-label="Pagos únicos realizados">
        ...
        <TableBody>
          {(rowsPerPage > 0
            ? sortedPayments.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
            : sortedPayments
          ).map(payment => (
            payment.status ?
              <TableRow key={payment.createdAt} className="table-row">
                <TableCell component="th" scope="row">
                  <Link to={`detail/${payment.accNumber ? 'account' : 'tdc'}/${payment.id}`}>{(payment.supplier.name).substring(0,12) + '...'}</Link>
                </TableCell>
                <TableCell>{setCurrency(payment.amount) + ' Bs.'}</TableCell>
                <TableCell>
