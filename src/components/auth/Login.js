...
const Login = withRouter(({ loginUser, isAuthenticated, setGoogleLogin, history }) => {
      ...
      <main className="form-content main-content">
        <div className="container">
          <section className="form-section">
            <h1>Inicia sesión <br /> a Pago Insibs</h1>
            <Formik initialValues={{ email: '', password: '' }} validationSchema={formSchema}
            ...
            {({ isValid, errors, touched }) => 
              <Form className="signin-form">
                ...
                <div className="form-group">
                  ...
                  <Field type="email" id="email" name="email" autoComplete="username" placeholder="johndoe@mail.com" className={`form-control ${touched.email && errors.email ? 'error' : ''}`} />
                ...
                <div className="form-group icon-group">
                  ...
                  <Field type="password" id="password" name="password" autoComplete="current-password" className={`form-control ${touched.password && errors.password ? 'error' : ''}`} />
                  <span className="fas fa-eye input-icon pointer" id="password-icon" onClick={togglePassword}/>
                  <ErrorMessage name="password">
                    {message => <span className="form-error"><i className="fas fa-warning"/> {message}</span>}
                  </ErrorMessage>
                </div>
                <Link to="/reset" className="pull-right">¿Olvidó su Contraseña?</Link>
  
                <div className="sign-in__submit clearfix">
                  <button className="button" disabled={!isValid} type="submit">Iniciar Sesión</button>
                  <GoogleLogin
